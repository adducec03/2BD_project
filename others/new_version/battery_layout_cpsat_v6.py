...
def solve_layout(coords: np.ndarray, S: int, P: int, R: float, tol: float,
    ...
    # 1) GRAFO base e assi
    E, A_bidir = build_graph(coords, R, tol)     # A_bidir = archi in entrambe le direzioni
    e1, e2 = pca_axes(coords)

    # 2) Euristica righe/colonne (serve per decidere l’asse dei "holes")
    ...
    # Profondità per evitare cicli: 0..P-1
    depth = {}
    for k in range(S):
        for i in range(N):
            depth[(i,k)] = model.NewIntVar(0, P-1, f"d[{i},{k}]")
            # se la cella non è usata, forza depth=0 (disattiva)
            model.Add(depth[(i,k)] == 0).OnlyEnforceIf(x[(i,k)].Not())
            # root -> profondità 0
            model.Add(depth[(i,k)] == 0).OnlyEnforceIf(r[(i,k)])
            # se è selezionata ma non è root, profondità ≥ 1
            not_root = model.NewBoolVar(f"not_root[{i},{k}]")
            model.Add(not_root == x[(i,k)] - r[(i,k)])
            model.Add(depth[(i,k)] >= 1).OnlyEnforceIf(not_root)

    # ogni arco scelto aumenta la profondità di 1 → niente cicli
    for (i, j) in A:
        for k in range(S):
            model.Add(depth[(j,k)] >= depth[(i,k)] + 1).OnlyEnforceIf(y[(i,j,k)])
...
def plot_solution(coords, R, S, x, z1, z2, E, L, solver,
    ...
    # --- asse guida opzionale ---
    if axis_vec is not None:
        axis = axis_vec / np.linalg.norm(axis_vec)
        c = coords.mean(axis=0)
        s_rel = (coords - c) @ axis
        pad = 2*R
        A = c + (s_rel.min() - pad) * axis
        B = c + (s_rel.max() + pad) * axis
        if start_end == "max":   # freccia dal lato 'max'
            A, B = B, A
        ax.plot([A[0], B[0]], [A[1], B[1]], linestyle="--", linewidth=2, color="black")
